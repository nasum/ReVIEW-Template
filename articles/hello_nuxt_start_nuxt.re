...
その後@<ttb>{Nuxt.js}の作法にしたがったディレクトリ構成でディレクトリを作成することで開発を進めることができます。ディレクトリ構成に関しては@<chapref>{hello_nuxt_directory}で詳しく見ていきます。
...
@<ttb>{npm}を使ってゼロから作成すると当たり前の話ですが自分で全て作らないといけないため、プロジェクト作成のスピードは遅くなりがちです。@<ttb>{vue-cli}を使い公式の@<ttb>{Nuxt.js}のテンプレートを使用することで必要なディレクトリ構成が初めから用意された状態でプロジェクトをはじめることができます。
...
@<ttb>{vue-cli}を使用してプロジェクトを開始していきます。@<ttb>{Nuxt.js}のテンプレートを使用してプロジェクトを開始していこうと思うのですが、@<ttb>{vue-clie}の@<ttb>{init}コマンドは@<ttb>{vue-cli}のバージョンが3系ではdeprecatedになっています。ですがまだ@<ttb>{Nuxt.js}のテンプレートが新しいものに対応されていないので@<ttb>{init}コマンドを使用できるようにしてテンプレートからプロジェクトを作成できるようにします。
...
以上@<ttb>{Nuxt.js}でのプロジェクト開始方法を解説していきました。何もない状態から作成するのであれば@<ttb>{vue-cli}を使ったほうがスピーディにプロジェクトを開始できるのでお薦めです。すでに@<ttb>{Vue.js}でアプリケーションを作っていて、それを@<ttb>{Nuxt.js}でSSRするなどの要件がある場合は@<ttb>{npm}でプロジェクトを開始するのがよいでしょう。各プロジェクトに適した形ではじめられるのがよいでしょう。