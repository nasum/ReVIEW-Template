...
@<ttb>{vue-cli}を使用してプロジェクトを開始していきます。@<ttb>{Nuxt.js}のテンプレートを使用してプロジェクトを開始していこうと思うのですが、@<ttb>{vue-cli}の@<ttb>{init}コマンドは@<ttb>{vue-cli}のバージョンが3系ではdeprecatedになっています。ですがまだ@<ttb>{Nuxt.js}のテンプレートが新しいものに対応されていないので@<ttb>{init}コマンドを使用できるようにしてテンプレートからプロジェクトを作成できるようにします。
